...
def analyze(symbol_filter: set = None):
    ...
    for f in files:
        ...
        # 4. 当日大阴线
        signals = df.filter(
            (pl.col("prev1_pct_chg") >= 0.095) &      # 前1日涨停
            (pl.col("prev2_pct_chg") >= 0.095) &      # 前2日涨停
            (pl.col("prev3_pct_chg") >= 0.095) &      # 前3日涨停
            (pl.col("open") > pl.col("prev_close")) & # 高开
            (pl.col("vol_ratio") > 1.5) &              # 放量
            (pl.col("body_pct") <= -0.03) &            # 大阴线
...
def main():
    parser = argparse.ArgumentParser(description="连续三涨停后高开放量大阴线策略统计")
    ...
    for index_code in args.index:
        ...
        if result is not None:
            print_stats(result, f"【{name}】连续3涨停后高开放量大阴线 → 收盘买入持有3日")
