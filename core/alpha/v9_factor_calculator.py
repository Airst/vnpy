...
class V9FactorCalculator(FactorCalculator):
    ...
    def build_features(self, padded_raw, col_map) -> dict[str, torch.Tensor]:
        ...
        # === V9: Stock-Level Crisis Factors ===
        # 1. Individual Stock's Downside Beta (Sensitivity to Crises)
        # How much does the stock drop when the market drops?
        mkt_ret_broad_neg = torch.clamp(mkt_ret_broad, max=0.0)
        stock_ret_neg = torch.clamp(ret_1, max=0.0)
        
        downside_cov = ts_mean(stock_ret_neg * mkt_ret_broad_neg, 20) - ts_mean(stock_ret_neg, 20) * ts_mean(mkt_ret_broad_neg, 20)
        downside_var_m = ts_mean(mkt_ret_broad_neg ** 2, 20) - ts_mean(mkt_ret_broad_neg, 20) ** 2
        features["downside_beta_20d"] = downside_cov / (downside_var_m + 1e-8)
        
        # 2. Crisis Alpha (Stock-specific resilience on market-down days)
        # Average stock return on days when market drops > 1%
        mkt_down_day = (mkt_ret_broad < -0.01).float()
        stock_ret_on_down = ret_1 * mkt_down_day
        down_day_count = ts_mean(mkt_down_day, 60)
        features["crisis_alpha_60d"] = ts_mean(stock_ret_on_down, 60) / (down_day_count + 1e-8)
