...
class V13FactorCalculator(FactorCalculator):
    """
    V13 因子计算器 — 方案2: 缩量确认 informed buyer bonus (防诱多)

    == 版本演进 ==
    V11 基线: informed buyer bonus 无过滤 (超跌 + 高kyle → +0.08)
    V13: bonus 加入缩量确认 (vol_shrink_ratio < 0.7)，区分真吸筹 vs 诱多

    == 当前状态 ==
    因子: 同 V11 基线
    标签改动: bonus 触发条件增加缩量过滤，只有"超跌+高kyle+缩量"才奖励
    原理: 真吸筹通常伴随缩量（筹码被锁），诱多往往放量

    == 设计决策 ==
    - vol_shrink_ratio = 5日均量 / 20日均量 < 0.7 → 显著缩量
    - 诱多通常放量，缩量确认可排除诱多场景
    - 如果缩量确认无效，说明诱多机制在日频数据中无法区分
    """
    ...
    def build_features(self, padded_raw, col_map) -> dict[str, torch.Tensor]:
        ...
        if gp_factors:
            print(f"[V13] Adding {len(gp_factors)} GP factors")
        ...
        # 超跌+知情交易者买入+缩量确认（方案2: 防诱多）
        vol_shrink_ratio = ts_mean(V, 5) / (ts_mean(V, 20) + 1e-8)
        drawdown_60d = (C - ts_max(C, 60)) / (ts_max(C, 60) + 1e-8)
        is_oversold = (drawdown_60d < -0.15).float()
        high_kyle = (features["kyle_lambda_20d"] > torch.nanmean(features["kyle_lambda_20d"], dim=0, keepdim=True)).float()
        is_shrink = (vol_shrink_ratio < 0.7).float()  # 显著缩量
        informed_buyer_bonus = is_oversold * high_kyle * is_shrink * 0.08
