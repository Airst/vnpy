...
class V14FactorCalculator(FactorCalculator):
    """
    V14 因子计算器 — 方案3: 10日标签降低噪声 (无 bonus)

    == 版本演进 ==
    V11 基线: 5日 beta-neutral label + informed buyer bonus
    V14: 去掉 bonus，改用 10日 excess return 降低标签噪声

    == 当前状态 ==
    因子: 同 V11 基线
    标签: 10日 beta-neutral excess return，无 bonus
    原理: 5日小盘股收益噪声大，拉长到10日可改善信噪比，让模型从纯收益中学习
    """
    ...
    def build_features(self, padded_raw, col_map) -> dict[str, torch.Tensor]:
        ...
        if gp_factors:
            print(f"[V14] Adding {len(gp_factors)} GP factors")
            features.update(gp_factors)

        # === Beta-Neutral Label (10-day, no bonus) ===
