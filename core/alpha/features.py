...
class AshareGPUDataset:
    """
    GPU-accelerated Factor Calculator for A-Shares using PyTorch.
    Replaces the previous string-based AlphaDataset definitions.
    """
    ...
    def __init__(self, device: str = "auto"):
        if device == "auto":
            ...
            self.device = device
        print(f"[FactorCalculator] Using device: {self.device}")
    ...
    def calculate_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        padded_raw = torch.full((num_stocks, max_len, len(cols)), float('nan'), device=self.device, dtype=torch.float32)
        ...
        raw_tensor = torch.from_numpy(raw_data).to(self.device)
        t_indices_t = torch.from_numpy(t_indices).to(self.device)
        s_indices_t = torch.from_numpy(s_indices).to(self.device)
        ...
        def ts_max(x, d):
            ...
            x_u = x.unsqueeze(1)
            # Pad left
            x_pad = F.pad(x_u, (d-1, 0), value=-float('inf')) 
